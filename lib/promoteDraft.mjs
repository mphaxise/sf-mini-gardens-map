...
function normalizeTrustTier(value) {
  const tier = String(value || "seedling").trim().toLowerCase();
  if (["seedling", "sprout", "canopy"].includes(tier)) {
    return tier;
  }
  return "seedling";
}
...
export function buildCanonicalEntryFromDraft(draft, options = {}) {
  ...
  return {
    ...
    submission: {
      ...
      contributor_anon_id: String(contributor.anon_id || "anon-unknown"),
      contributor_alias: String(contributor.public_alias || "Anonymous Seedling"),
      contributor_trust_tier: normalizeTrustTier(contributor.trust_tier)
    }
  };
}
